...
class TB_Free_Gift_Helpers {
	...
	public static function get_valid_gifts() {
		...
		$gift_ids = self::convert_string_to_array( get_option( 'woocommerce_tb_free_gift_ids' ) );
		...
		$gift_qty         = self::convert_string_to_array( get_option( 'woocommerce_tb_free_gift_quantity' ) );
	...
	public static function is_correct_logged_condition() {
		...
		if ( 'yes' !== get_option( 'woocommerce_tb_free_gift_only_for_authorized' ) ||
		     (
			     'yes' === get_option( 'woocommerce_tb_free_gift_only_for_authorized' ) &&
			     is_user_logged_in()
		     )
		) {
			return true;
	...
	public static function is_cart_total_enough() {
		...
		$order_total_type = esc_attr( get_option( 'woocommerce_tb_free_gift_order_total_type' ) );
	...
	public static function is_correct_cart_total_condition() {
		...
		self::$minimal_order_amount = abs( floatval( get_option( 'woocommerce_tb_free_gift_minimum_order_amount' ) ) );
