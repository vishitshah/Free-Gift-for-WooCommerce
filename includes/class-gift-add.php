...
class TB_Free_Gift_Add {
	...
	public static function gift_checkout_process( $cart ) {
		...
		self::$is_correct_logged_condition = TB_Free_Gift_Helpers::is_correct_logged_condition();
		...
		$gifts = TB_Free_Gift_Helpers::get_valid_gifts();
	...
	public static function check_cart_total_condition() {
		self::$is_correct_cart_total_condition = TB_Free_Gift_Helpers::is_correct_cart_total_condition();
	}
	...
	public static function clean_cart( $cart ) {
		...
		if ( TB_Free_Gift_Helpers::is_only_gifts_in_cart( self::$gift_cart_ids ) ) {
			$cart->empty_cart();
		}
	}
	...
	public static function add_gift_label_in_cart( $product_name, $cart_item, $cart_item_key ) {
		...
		if ( 'yes' !== get_option( 'woocommerce_tb_free_gift_show_gift_label_in_cart' ) ) {
			return $product_name;
		}
		...
		// If current product is a gift.
		if ( in_array( $cart_item_key, self::$gift_cart_ids ) ) {
			$product_name_postfix = '<span class="tb_free_gift_label">' . apply_filters(
					'tb_free_gift_product_name_postfix',
					sprintf( ' - %s', esc_html__( 'Free Gift', 'free-gift-for-woocommerce' ) )
				) . '</span>';
		}
	...
	public static function add_gift_message() {
		...
		if ( ! TB_Free_Gift_Helpers::is_any_gift_in_cart( self::$gift_cart_ids ) ) {
			return;
		}
		...
		$gift_message = sanitize_text_field( get_option( 'woocommerce_tb_free_gift_message' ) );
